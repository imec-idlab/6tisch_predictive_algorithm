...
REGEXP_PRED_ENER = re.compile('^.*?INFO:\sApp\s\s\s\s\s\s\s]\sPREDICT\s(?P<time>([0-9]+))\sms,\sRX\s(?P<RX>([0-9]+))\sms,\sTX\s(?P<TX>([0-9]+))\sms')
...
def read_logline(line):
    ...
    # Prediction update
    matchpred = re.match(REGEXP_PRED_ENER, chomped_line)
    if(matchpred):
        t = float(matchpred.group("time"))
        rx = float(matchpred.group("RX"))
        tx = float(matchpred.group("TX"))
        for n in nodes.nodes:
            n.predtime = t
        root = nodes.search_node(ROOT)
        if(root != -1):
            root.energest.append([t,rx,tx])
        nodes.read = False
    # INT update
    if(nodes.mop == 1):
    ...
    if(matchint):
        ...
        if((node.updtime == None) or ((((node_asn_ls4b+node_asn_ms1b*pow(2,32))*10)+config.COOJA_OFFSET) > (node.updtime + INT_INTERVAL))):
            ...
            if(nodes.mop == 1):
                ...
                voltage = int(matchint.group("voltage"))
                p_harv = int(matchint.group("p_harv"))
                e_asn_ms1b = (int)(matchint.group("e_asn_ms1b"))
                e_asn_ls4b = (int)(matchint.group("e_asn_ls4b"))
                node.last_voltage = voltage
                node.last_p_harv = p_harv
                node.last_harvester_update = ((e_asn_ls4b+e_asn_ms1b*pow(2,32))*10)+config.COOJA_OFFSET
